...
// Function to fetch movie data using the stored ID
async function fetchDataUsingId() {
  // Retrieve the stored ID from sessionStorage
  const storedId = sessionStorage.getItem(sessionId);
  // Fetch movie data using the stored ID and API key
  const data = await fetchApi(`https://api.themoviedb.org/3/movie/${storedId}?api_key=${API_KEY}`);  
...
async function showDetails() {
  // Fetch movie data using the stored ID
  const movie = await fetchDataUsingId();
  
  // Populate the details grid with movie details
...
// Call the showDetails function to display movie details
showDetails();